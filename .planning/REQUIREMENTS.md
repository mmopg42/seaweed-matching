...
- [ ] JSON 형식으로 상태 반환
  ...
  ```
- [ ] 현재 실행 중인 시뮬레이션 ID 추적
- [ ] 완료 여부를 불리언이 아닌 상태로 반환

### LOG-01: 동적 로그 경로 해결
- [ ] `%APPDATA%\ChronoView\Logs\` 하위에서 최신 폴더 자동 탐색
- [ ] 날짜별 폴더 `{YYYYMMDD}` 자동 검색 로직
- [ ] log-analyst 에이전트에 동적 경로 해결 추가
- [ ] Windows/WSL 경로 호환성 유지

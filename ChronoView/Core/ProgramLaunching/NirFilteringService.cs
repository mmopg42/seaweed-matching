...
/// <summary>
/// NIR 필터링 서비스: NIR spectrum 파일을 모니터링하고 필터링하여 조건에 따라 이동/삭제 처리
/// </summary>
public class NirFilteringService : IDisposable
{
    private readonly ILogger<NirFilteringService> _logger;
    ...
    public NirFilteringService(
        ILogger<NirFilteringService> logger,
    ...
    public async Task<(bool Success, string Message)> StartFilteringAsync()
    {
        ...
        try
        {
            ...
            if (!Directory.Exists(monitorPath))
            {
                ...
                _logger.LogInformation("Created monitor directory: {Path}", monitorPath);
            }
    ...
    private void OnFileCreated(object sender, FileSystemEventArgs e)
    {
        try
        {
            ...
            _logger.LogDebug("New NIR file detected: {FileName}", Path.GetFileName(e.FullPath));
    ...
    private void ProcessFile(string txtFilePath)
    {
        try
        {
            ...
            {
                // 조건 만족: 파일 이동
                _logger.LogDebug("✓ {FileName} - {Message}", fileName, result.Message);
                ...
                if (result.CriteriaDetails != null && result.CriteriaDetails.Count > 0)
                {
                    ...
                    {
                        _logger.LogDebug("  {CriterionName}: {Value:F5} (threshold: {Threshold:F5}) - {Status}", 
                            criterion.Key, 
                ...
                _logger.LogDebug("  → Moved: {FileName} → {DestPath}", fileName, destinationPath);
                ...
                if (hasSpcFile)
                {
                    ...
                    _logger.LogDebug("  → Paired .spc file also moved: {SpcFileName}", spcFileName);
                }
            }
            else
            {
                // 조건 불만족: 파일 삭제
                _logger.LogDebug("✗ {FileName} - Filter FAILED (criteria not met)", fileName);
                
                File.Delete(txtFilePath);
                _logger.LogDebug("  → Deleted: {FileName}", fileName);

                if (hasSpcFile)
                {
                    File.Delete(spcFilePath);
                    _logger.LogDebug("  → Paired .spc file also deleted");
                }
            }
        }
        catch (Exception ex)

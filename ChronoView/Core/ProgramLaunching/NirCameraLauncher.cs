...
/// <summary>
/// NIR Camera 1 프로그램 실행 및 상태 추적 담당
/// </summary>
public class NirCameraLauncher : IDisposable
{
    ...
    public async Task<(bool Success, string Message)> LaunchAsync()
    {
        ...
        {
            _logger.LogError(ex, "Failed to launch NIR Camera 1");
            return (false, $"Error launching NIR Camera 1: {ex.Message}");
        }
    }
        ...
        Task.Run(async () =>
        {
            try
            {
                if (_process != null)
                {
                    ...
                    StatusChanged?.Invoke(this, false);
                    _logger.LogInformation("NIR Camera 1 deactivated (process exited)");
                }
            }

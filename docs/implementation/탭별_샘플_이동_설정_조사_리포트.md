...
### Phase 3: 이동 로직
7. Branch `ExecuteMoveWithConfirmation` by tab.
8. Implement `ExecuteMoveForLineAsync` helper.
9. Combined tab runs sequentially (Line1 -> Line2).

### Phase 4: 구조화 및 개선
10. Introduce `LineMoveContext` and `LineMoveExecutor` if further refactoring is needed.
11. Implement `ILineMoveSettingsManager` if settings management needs to be isolated.
12. Add test scripts.

...
### 내부 모듈
- `config_manager`: 설정 관리 (`ConfigManager`)
- `group_manager`: 그룹 매칭 로직 (`GroupManager`)
- `file_matcher`: 파일 매칭 엔진 (`FileMatcher`, `FileMatcherWorker`)
- `file_operations`: 파일 작업 처리 (`FileOperationWorker`)
- `delete_manager`: 삭제 관리 (`DeleteManager`)
- `image_loader`: 이미지 비동기 로딩 (`ImageLoaderWorker`, `prefetch_images`)
- `file_count_worker`: 파일 개수 카운트 (`FileCountWorker`)
- `ui_components`: UI 컴포넌트 (`FlowLayout_`)
- `tooltips`: 툴팁 관리 (`setup_tooltips`, `clear_tooltips`)
- `log_panel`: 로그 패널 (`LogPanel`)
- `utils`: 유틸리티 함수
- `path_utils`: 경로 유틸리티 (`get_normal_thumbnail_path`, `extract_date_from_paths`, `auto_update_paths_with_date`)
- `image_registry`: 이미지 레지스트리 (`ImageRegistry`)
- `window_state_manager`: 윈도우 상태 관리 (`WindowStateManager`)
- `view_manager`: 뷰 관리 (`ViewManager`)
- `statistics_calculator`: 통계 계산 (`StatisticsCalculator`)
- `abnormal_detector`: 이상치 감지 (`AbnormalDetector`)
...
- **초기화 변수**:
  - `drag_start_pos`: 드래그 시작 위치
  - `is_dragging`: 드래그 중 여부

#### `mousePressEvent(event)`
- **설명**: 마우스 클릭 시작 처리
- **동작**: 왼쪽 버튼 클릭 시 드래그 시작 위치 기록

#### `mouseMoveEvent(event)`
- **설명**: 드래그 중 행 선택
- **동작**: 마우스 이동 시 시작 위치부터 현재 위치까지의 행 선택
...
- **매개변수**:
  - `start_idx`: 시작 인덱스
  - `end_idx`: 종료 인덱스
- **동작**: MonitorRow의 선택 상태 변경

**용도**: 마우스 드래그로 여러 데이터 그룹을 동시에 선택할 수 있는 기능 제공

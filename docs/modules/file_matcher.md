...
**파일 경로**: `script/domain/file_matcher.py`  
**파일 크기**: 374 라인  
**총 클래스**: 3개 (`Communicate`, `FolderEventHandler`, `FileMatcher`)  
**업데이트**: 2025-12-04
...
### 1. 모니터링 중지 기능 강화
- **`is_enabled` 플래그 추가**: FileMatcher를 활성화/비활성화 가능
- **Stop 버튼 지원**: 모니터링 중지 시 파일 처리 중단
- **메서드 가드**: `add_or_update_file()`, `remove_from_unmatched()`에서 `is_enabled` 체크

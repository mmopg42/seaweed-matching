...
**파일 경로**: `script/services/file_operations.py`  
**파일 크기**: 539 라인  
**총 클래스**: 1개 (`FileOperationWorker`)  
**업데이트**: 2025-12-04
...
- **pyqtSignal → Signal**: 시그널 클래스 변경

### 2. Import 경로 변경
- **Before**: `from config_manager import ConfigManager`
- **After**: `from infrastructure.config_manager import ConfigManager`
...
metadata_ready = Signal(dict)          # 메타데이터 준비 완료
file_conflict = Signal(str, str, str)  # (파일/폴더명, src, dst)
```

**변경사항 (PyQt6 → PySide6)**:
- `pyqtSignal` → `Signal`
- 시그널 구조 변경 (로그 중심으로 간소화)

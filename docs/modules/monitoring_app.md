...
### 1. ImageRegistry 공유 문제 해결
- **문제**: `MainWindow`와 `ImageManager`가 각각 독립적인 `ImageRegistry` 생성
- **해결**: `MainWindow`에서 생성한 `ImageRegistry`를 `ImageManager`에 주입
- **효과**: 위젯 등록이 올바르게 동기화되어 이미지 업데이트 정상 작동

### 2. 성능통계 다이얼로그 모달리스 변경
- **문제**: 성능통계 다이얼로그가 모달로 열려 메인 윈도우와 상호작용 불가
- **해결**: `exec()` → `show()` + `NonModal` 설정
- **효과**: 성능통계를 보면서 메인 윈도우 조작 가능

### 3. 디버그 로깅 추가
- 레지스트리 공유 확인 로그 (메모리 주소 비교)
- 위젯 등록 시점 로그
- UI 생성 완료 후 레지스트리 상태 로그

---

## 🔥 이전 변경사항 (2025-12-03 ~ 2025-12-04)
